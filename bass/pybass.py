...
if getattr(sys, 'frozen', False):
    # Запуск в скомпилированном бинарнике
    app_path = os.path.dirname(sys.executable)
else:
    # Запуск в режиме разработки (исходный код)
...
if platform.system().lower() == 'windows':
    ...
    if not os.path.isfile(fn):
        BASS_LIB_DIR = app_path
        fn = os.path.join(BASS_LIB_DIR, 'bass.dll')
    bass_module = ctypes.WinDLL(fn)
    func_type = ctypes.WINFUNCTYPE
else:
    fn = os.path.join(BASS_LIB_DIR, 'libbass.so')
    if not os.path.isfile(fn):
        BASS_LIB_DIR = app_path
        fn = os.path.join(BASS_LIB_DIR, 'libbass.so')
        if not os.path.isfile(fn):
            BASS_LIB_DIR = '/usr/lib'
            fn = os.path.join(BASS_LIB_DIR, 'libbass.so')
    bass_module = ctypes.CDLL(fn, mode=ctypes.RTLD_GLOBAL)
